...
""" Image data generator for DB1 NinaPro for PartB"""
...
class DataGeneratorB(keras.utils.Sequence):
    ...
    def __init__(self, repetitions, input_directory, batch_size=32, sample_weight=False, dim=(15,10,3),
                 classes=2, shuffle=True, zero_column=0,noise_column = 0,
                 snr_db=0, window_size=15,scale_sigma=0,window_step=1,rotation=0, rotation_mask=None, time_warping=0, mag_warping=0, permutation=0,
                 data_type='rms', partb_zero =False,partb_noise=False,
    ...
    def __data_generation(self, indexes):
        ...
        for k, index in enumerate(indexes):
            ...
            if self.window_size != 0:
                ...
                # Create 3 windows for 3-channel-image
                ...
                #Compensate for windowing to match dimensions
                if self.dim ==(15,10,3):
                    if im1.shape != (15,10):
                        a=np.zeros(10)
                        im1=np.insert(im1,14,a,0)
                    if im3.shape != (15,10):
                        a=np.zeros(10)
                        im3=np.insert(im3,14,a,0)
               #                        
                #Create image with 3 channels along one axis       
                image =(im1,im2,im3)
            ...
            if self.partb_zero is True :
                x_aug[:,self.zero_column] = 0                            
                                        
            #PartB2 - Input random noise to electrode    
            
            if self.partb_noise is True:
                scale=0.1                   
                x_aug[:,self.noise_column,:]=scale*np.random.randn()  
    ...
    def __load_dataset(self):
        '''Loads data and applies preprocess_function'''
        X, y, r = [], [], []
        self._max_len = 0
...
#                           
                ...
                if 0 in self.classes:
                    for rep, label in rest_rep_groups:
                        ...
                        if self.preprocess_function is not None:
                            if isinstance(self.preprocess_function, list):
                                for params, func in zip(self.preprocess_function_1_extra, self.preprocess_function):
                                    x = func(x, **params)
