...
"""Train customized VGG19 - DB1 NINAPRO - A EXERCISE""" 
...
from utils import *
from datagenerator_db1 import*
from vgg19_db1ninapro import *
...
with open('DB1_vgg19.json') as json_file:
    config_data = json.load(json_file)


LOG_FILE = os.path.abspath('drive/Thesis_emg'+'/'+
        'logs_vgg19') + os.sep + TIMESTAMP + '_' + config_data['logging']['log_file'] + '.log'
LOG_TESNORBOARD_FILE = os.path.abspath('drive/Thesis_emg'+'/'+
        'tensorboardlogs_vgg19') + os.sep + TIMESTAMP + '_' + config_data['logging']['log_file']

MODEL_SAVE_FILE = os.path.abspath('drive/Thesis_emg'+'/'+
        'models_vgg19') + os.sep + TIMESTAMP + '_' + config_data['model']['save_file'] + '_{}.json'
        
MODEL_WEIGHTS_SAVE_FILE = os.path.abspath('drive/Thesis_emg'+'/'+
        'models_vgg19') + os.sep + TIMESTAMP + '_' + config_data['model']['save_file'] + '_{}.h5'

METRICS_SAVE_FILE = os.path.abspath('drive/Thesis_emg'+'/'+
    'metrics_vgg19') + os.sep + TIMESTAMP + '_' + config_data['logging']['log_file'] + '.mat'
...
PARAMS_TRAIN_GENERATOR['preprocess_function'] = [preprocessing.lpf]
PARAMS_TRAIN_GENERATOR['preprocess_function_extra'] = [{'fs':100}]
PARAMS_TRAIN_GENERATOR['data_type'] = 'rms'
PARAMS_TRAIN_GENERATOR['classes'] = [i for i in range(13)]

PARAMS_VALID_GENERATOR['preprocess_function'] = [preprocessing.lpf]
PARAMS_VALID_GENERATOR['preprocess_function_extra'] = [{'fs':100}]
...
#Select optimizer for fine tuning 
optimizer = optimizers.SGD(lr=PARAMS_TRAINING['fine_tuning_lrate'], momentum=0.9,nesterov=True)
opt='SGD'
...
valid_generator = DataGenerator(input_directory=input_directory, **PARAMS_VALID_GENERATOR)
X_valid, Y_valid, valid_reps = valid_generator.get_data()    
optimizer = optimizers.SGD(lr=PARAMS_TRAINING['training_lrate'],momentum=0.9,nesterov=True)
...
fig.savefig('/accuracy-{}-model-{}-lr-{}-optimizer-{}-batch_size-{}-snr..jpg'.format(MODEL,PARAMS_TRAINING['l_rate'], opt,PARAMS_TRAIN_GENERATOR['batch_size'],PARAMS_TRAIN_GENERATOR['noise_snr_db']))
plt.show()
plt.close(fig)
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
fig.savefig('/loss-{}-model-{}-lr-{}-optimizer-{}-batch_size-{}-snr..jpg'.format(MODEL,PARAMS_TRAINING['l_rate'], opt,PARAMS_TRAIN_GENERATOR['batch_size'],PARAMS_TRAIN_GENERATOR['noise_snr_db']))
plt.show()
plt.close(fig)
