...
"""Train customized VGG16 - DB1 NINAPRO - A EXERCISE""" 
...
with open('DB1_vgg16_f1.json') as json_file:
    config_data = json.load(json_file)


LOG_FILE = os.path.abspath(
        'logs_vgg16') + os.sep + TIMESTAMP + '_' + config_data['logging']['log_file'] + '.log'
LOG_TESNORBOARD_FILE = os.path.abspath(
        'tensorboardlogs_vgg16') + os.sep + TIMESTAMP + '_' + config_data['logging']['log_file']

MODEL_SAVE_FILE = os.path.abspath(
        'models_vgg16') + os.sep + TIMESTAMP + '_' + config_data['model']['save_file'] + '_{}.json'
        
MODEL_WEIGHTS_SAVE_FILE = os.path.abspath(
        'models_vgg16') + os.sep + TIMESTAMP + '_' + config_data['model']['save_file'] + '_{}.h5'

METRICS_SAVE_FILE = os.path.abspath(
    'metrics_vgg16') + os.sep + TIMESTAMP + '_' + config_data['logging']['log_file'] + '.mat'
...
for i in range(0,11):                   #Freeze 11 first layers for VGG16
...
fig.savefig('/accuracy-{}-model-{}-optimizer-{}-batch_size-{}-snr..jpg'.format(MODEL, PARAMS_TRAINING['optimizer'],PARAMS_TRAIN_GENERATOR['batch_size'],PARAMS_TRAIN_GENERATOR['snr_db']))
plt.show()
plt.close(fig)
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
fig.savefig('/loss-{}-model-{}-optimizer-{}-batch_size-{}-snr..jpg'.format(MODEL,PARAMS_TRAINING['optimizer'],PARAMS_TRAIN_GENERATOR['batch_size'],PARAMS_TRAIN_GENERATOR['snr_db']))
plt.show()
plt.close(fig)
